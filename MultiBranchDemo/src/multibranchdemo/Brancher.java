...
/**
 * A simple example of a wizard with multiple branch points.  It provides
 * the following sequence of pages:
 * <pre>
 *      /-C1-D1-E1-F1
 *     / 
 *  A-B
 ...
 *             \-E2b-F2b-G2b
 * </pre>
 * Each letter above is a WizardPage subclass with some content on it.
 * Two of them, B and D2 contain checkboxes.  Notice that all we do is set
 * the name of the checkbox - this causes it to be listened to automatically,
 * and its selected state put into the data map passed into 
 * getWizardForStep().
 * <p/>
 * A branching wizard is really a wrapper for multiple wizards (which may
 * have their own branches, or simply a fixed set of panels.  We pass at
 * least an initial panel to the super constructor;  when the last page of
 * the initial steps is hit, getWizardForStep() is called to decide what
 * wizard to return for the next sequence.
 *
 * @author Tim Boudreau
 */
public class Brancher extends WizardBranchController {
    ...
    Class[] c1Sequence = new Class[]{
        C1.class, D1.class, E1.class, F1.class, 
    };
    ...
    Class[] e2aSequence = new Class[]{
        E2a.class, F2a.class, G2a.class,
    };
    Wizard e2a = WizardPage.createWizard (e2aSequence, WizardResultProducer.NO_OP);
    
    //This will be the other secondary branch
    Class[] e2bSequence = new Class[] {
        E2b.class, F2b.class, G2b.class,
    };
    ...
    public Wizard getWizardForStep (String step, Map data) {
       ...
       if ("multibranchdemo.Brancher$B".equals(step)) {
            if (Boolean.TRUE.equals(data.get("foo"))) { //check some data in the map to decide
                return c1;
            } else {
                return secondBranch;
            }
    ...
    private class Brancher2 extends WizardBranchController {
        ...
        public Wizard getWizardForStep (String step, Map data) {
            if ("multibranchdemo.Brancher$D2".equals(step)) {
                if (Boolean.TRUE.equals(data.get("bar"))) { //check some data in the map to decide
                    return e2a;
                } else {
                    return e2b;
                }
